...
3. `cosign copy` from `lw-poc-build` onto the prod digest, then `cosign sign --key` the prod ImageStream (`oc tag` does not copy `.sig` tags).
4. In **this** repository, replace `image.digest` and set `replicas: 1`.
5. Commit and push to `student_prod_gitops_repo_url`.
6. Confirm Argo CD Application `lw-poc-prod` is Synced/Healthy and
