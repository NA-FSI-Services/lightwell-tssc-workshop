...
This page is a *simulated media drop* on Showroom. The claim stays on the network. There is no USB, no data diode, and no `dd` to physical media.

Ungated. No Validate Job.

The scored key-based signature is xref:module-14-disconnected-verify.adoc[5.3] (`~/lab-trust`). This appendix is what you would *carry* to a verifier that cannot reach Fulcio or the TUF Route.
...
|----------|--------------------------------------|------------------------|
| TUF `root.json` | Keyless verify without the live TUF Route | `tuf_url_hint` `/root.json` |
| Fulcio CA PEM | Keyless identity / admission materials | Secret `rhtas-tuf-keys` in `tssc-admission` |
| Rekor public key | Transparency log verify when a tlog exists | Same Secret |
| `cosign.pub` | Key-based verify from xref:module-14-disconnected-verify.adoc[5.3] | `~/lab-trust/cosign.pub` |
| App digest | What you verify (the 5.1 BuildConfig image) | ImageStream in `lw-poc-build` |
| SBOM (optional) | Inventory on the far side of the diode | 4.4 / 7.1 workspace or TPA export |
...
* xref:module-14-disconnected-verify.adoc[5.3 Disconnected verify] — scored `--key` sign + verify
* xref:module-12-sign-keyless.adoc[5.1 Sign (keyless)] — connected Fulcio path
