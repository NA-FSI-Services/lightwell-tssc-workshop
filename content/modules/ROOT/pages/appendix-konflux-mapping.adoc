...
| Column | Meaning |
|--------|---------|
| *Client-engagement name* | Word a customer or engagement draft may use. You still say it in the room; you do not install that product here. |
| *What this claim runs* | Cluster object under its *real* OpenShift / Tekton name. |
| *Where you clicked* | Gated module, callout, or not on this claim. |
...
|Hermeto prefetch
|Tekton Task `prefetch-dependencies` in namespace `lightwell-tasks`. Maven `dependency:go-offline` then `mvn -o` against in-cluster Nexus. Do not pull `quay.io/konflux-ci/hermeto`. Do not name the cluster object `hermeto`. Diagram: xref:module-09-prefetch.adoc[4.2].
|xref:module-09-prefetch.adoc[4.2] (scored)

|Buildah
|OpenShift *BuildConfig* started from Tekton (`oc start-build --from-dir`). No Buildah task on the scored path.
|xref:module-11-build-sbom.adoc[4.4] (scored)
...
|Tekton Chains
|RHTAS *keyless* cosign Task `cosign-sign-keyless` (xref:module-12-sign-keyless.adoc[5.1]). There is *no* Tekton Chains on this claim and no SLSA L3. xref:module-13-attest-conforma.adoc[5.2] leaves `skip-att-sig-check=true`.
...
|xref:module-13-attest-conforma.adoc[5.2] (scored)
...
* xref:module-09-prefetch.adoc[4.2 Prefetch] — Hermeto callout and diagram
* xref:module-11-build-sbom.adoc[4.4 Build and SBOM] — Buildah → BuildConfig
* xref:module-07-live-renovate.adoc[3.3 Live Renovate] — MintMaker → this bot
