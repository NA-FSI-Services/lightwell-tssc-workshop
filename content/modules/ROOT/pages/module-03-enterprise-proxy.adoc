...
Honor system: later modules still open if you skip this. When Validate Jobs ship, they grade the ConfigMap (Nexus host, no `REPLACE_ME`, not the worked-example URL) plus a short report. The `~/track-2-1-settings.xml` copy is for `mvn -s`.
