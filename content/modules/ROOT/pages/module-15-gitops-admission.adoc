...
TSSC runtime enforcement is: only the signed digest runs. 6.1 is the observation that GitOps sets that digest on stage, and unsigned deny is a *pull* failure (`ImagePullBackOff`), not `oc apply` stderr. Lightwell pins inside the image do not admit an unsigned tag.

You learn TrustPolicy (`enforce: true`, live digest, pipeline SA subject) plus `cosign copy` after `oc tag` because signatures do not travel with `oc tag` by themselves. Prod Argo stays on the prod remote.
...
* Observe TSSC’s admit-and-promote control: stage GitOps runs the signed digest; an unsigned tag on the same ImageStream is denied at pull
* `oc tag` the signed image into `lw-poc-staging`, `cosign copy` signatures, and commit digest + `replicas: 1` to the *stage* GitOps remote
...
* TSSC admission is namespaced `ImagePolicy` from one learner TrustPolicy (not `ClusterImagePolicy`).
* Lightwell Network / Clearinghouse Premier GAVs do not bypass unsigned deny.
