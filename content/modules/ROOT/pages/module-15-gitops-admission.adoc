...
Native `ImagePolicy` `signedEmail` must be an email. Track 5.1 identity is a Kubernetes SA URI, so the CronJob renders `policyType: PublicKey` from ConfigMap `lab-cosign-pubkey` (the 5.3 `cosign.pub` you published). Keep the 5.1 SA URI in TrustPolicy — that is still the Fulcio identity. `oc tag` does not copy `.sig` tags; `cosign copy` (and a dest-repo `cosign sign --key`) after the tag.
...
* `oc tag` the signed image from `lw-poc-build` into `lw-poc-staging`, then `cosign copy` signatures onto that dest ImageStream
* Commit digest + `replicas: 1` to the *stage* GitOps remote
* Fill TrustPolicy so no `REPLACE_ME_*` remains and `enforce: true` (subject stays the 5.1 SA URI)
...
* SA URI subjects render `PublicKey` from 5.3 `lab-cosign-pubkey`; `oc tag` needs `cosign copy`.
