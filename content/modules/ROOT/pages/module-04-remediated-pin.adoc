...
Lightwell Clearinghouse Premier can remediate *member-specific* versions that are not in the public Network catalog. The TSSC control is identical: read the `fixed` event, pin that exact version from the enterprise proxy, never bump to a newer upstream line. This lab’s scored GAV is the Network sample `3.14.0.rhlw-00001`.
...
* State that Clearinghouse Premier member-specific versions would pin the same way once they exist in the proxy
...
* Clearinghouse Premier member versions use the same pin mechanic.
