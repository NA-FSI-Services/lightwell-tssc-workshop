...
Lab pedagogy (fresh-claim negatives, worked-example fail, report keys) stays on the gated pages and future Validate Jobs. Do not re-score those here.
...
|xref:module-01-hummingbird-verify.adoc[1.1], xref:module-02-hummingbird-mirror.adoc[1.2]
...
|`~/track-2-1-settings.xml` shows in-cluster Nexus host. Default `<commons.lang3.version>` is `3.14.0.rhlw-00001` (not an unused profile, not `LW-DEMO-0001` / spring-core).
|xref:module-03-enterprise-proxy.adoc[2.1], xref:module-04-remediated-pin.adoc[2.2]
...
|xref:module-05-golden-path-source.adoc[3.1], xref:module-06-bind-base-deps.adoc[3.2], xref:module-07-live-renovate.adoc[3.3]
...
|Active Dockerfile/`pom` have no public `FROM` / Central / `curl`. Pipeline includes Task `prefetch-dependencies`. `build-egress` on `lw-poc-build` is not the too-open seed. Image + SBOM artifacts exist.
|xref:module-08-forbid-list.adoc[4.1], xref:module-09-prefetch.adoc[4.2], xref:module-10-build-networkpolicy.adoc[4.3], xref:module-11-build-sbom.adoc[4.4]
...
|xref:module-12-sign-keyless.adoc[5.1], xref:module-13-attest-conforma.adoc[5.2], xref:module-14-disconnected-verify.adoc[5.3]
...
|TPA document for the promoted digest. `~/track-7-2-blast-radius.txt` has `LW-DEMO-0002`, `3.14.0.rhlw-00001`, `via-lightwell-pin`. `acs-image-check` is `passed` or `failed`, not `skipped`.
...
* xref:index.adoc[Workshop home] — learner map; this checklist is after, not a Day-1 exam
