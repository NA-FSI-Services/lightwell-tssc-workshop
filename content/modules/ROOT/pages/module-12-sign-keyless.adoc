...
Sign the *app* digest you built in xref:module-11-build-sbom.adoc[4.4] — not the Hummingbird base. This Check is *keyless* RHTAS (Fulcio, Rekor, TUF) plus `cosign verify` on *that* digest. Key-based / mirrored-TUF verify is a *second* Check (5.3 / V2-40). Do not treat keyless-only as Track 5 done.
...
* Discover Fulcio, Rekor, and TUF from cluster userinfo (do not invent Route hosts)
* Get `cosign-sign-keyless` to Succeeded on the learner-built app image
* `cosign verify` that digest with the pipeline SA identity and Kubernetes OIDC issuer
* Leave TUF-on-disk / `--key` verify for 5.3

== Keyless vs keys

// VISUAL: https://github.com/NA-FSI-Services/lightwell-tssc-workshop/issues/65
image::FIXME-keyless-rhtas-vs-keys.png[Keyless RHTAS Fulcio Rekor TUF versus later key-based verify,width=700,link=self,window=blank]

Keyless is the *connected* PoV path: the pipeline SA presents an OIDC token, Fulcio issues a short-lived cert, Rekor records the signature, TUF distributes the trust root. Customers who cannot reach Fulcio at verify time still need 5.3.
...
If 4.4 already ran `cosign-sign-keyless` to Succeeded, skip to verify. Otherwise edit `.tekton/pipelinerun.yaml` on `lw-student/spring-boot-lw-poc`: replace Fulcio / Rekor / TUF with the userinfo URLs, `STUDENT_REPO_URL_PLACEHOLDER` with `student_repo_url`, and `<lab-namespace>` with `lw-poc-build`. Then create a new run:
...
image::FIXME-pipelinerun-cosign-sign-keyless.png[PipelineRun graph with cosign-sign-keyless Succeeded,width=700,link=self,window=blank]

Do *not* `oc tag` into staging here (Track 6). Do *not* copy TUF onto a lab trust path (5.3).
...
Python signs the same way (RHTAS keyless on the FastAPI digest). Not scored here.

== Key takeaways

* Keyless is the connected path; 5.3 still exists for disconnected *procedure*.
* You signed the *app*, not only Hummingbird.
