...
* Leave key-based sign + `--key` verify for xref:module-14-disconnected-verify.adoc[5.3]
...
Keyless is the *connected* PoV path: the pipeline SA presents an OIDC token, Fulcio issues a short-lived cert, Rekor records the signature, TUF distributes the trust root. Customers who cannot reach Fulcio at verify time still need xref:module-14-disconnected-verify.adoc[5.3].
...
Do *not* `oc tag` into staging here (Track 6). Do *not* generate a Showroom key pair or `cosign sign --key` here (xref:module-14-disconnected-verify.adoc[5.3]).
...
* Keyless is the connected path; xref:module-14-disconnected-verify.adoc[5.3] adds a key-based signature on the same digest.
