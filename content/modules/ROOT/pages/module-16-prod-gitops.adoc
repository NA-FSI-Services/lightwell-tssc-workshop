...
* `oc tag` the signed image into `lw-poc-prod`, `cosign copy` (and dest `cosign sign --key`) onto that ImageStream, then commit digest + `replicas: 1` to the *prod* GitOps remote
