...
TSSC promotion is a *deliberate git commit* to production desired state, not retargeting Argo at stage. 6.2 is the observation that `lw-poc-prod` sources the prod remote, ImagePolicy from 6.1 still holds, and the digest is the one you signed.

You learn a second clone and `cosign copy` onto the prod ImageStream because `oc tag` still does not copy `.sig` tags. Konflux Release Service maps to this commit.
...
* Observe TSSC’s production-promote control: the same signed digest is committed to a *prod* GitOps remote; Argo does not track stage
* `oc tag` into `lw-poc-prod`, `cosign copy` (and dest `cosign sign --key`), then set digest + `replicas: 1` on the prod remote
* Keep `lw-poc-prod` `.spec.source.repoURL` on the prod remote; leave the stage Application and TrustPolicy file alone
...
* TSSC promotion is a git commit to a second remote, not a second values file and not retargeting prod Argo at stage.
* Lightwell Network / Clearinghouse Premier ride inside the digest. They are not a different promote button.
