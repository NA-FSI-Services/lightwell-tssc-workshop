...
Core learners complete xref:module-04-remediated-pin.adoc[2.2] with the scored OSV and do *not* need this appendix. Use it to teach the *field PoV automation implication*: new remediated OSV files can trigger tickets or rebuild pipelines without binding the workshop to any customer ITSM product.
====
...
When the manifest checksum changes, something new (or updated) landed in the OSV path—often a new `fixed` event naming a `.rhlw-*` pin you can feed into xref:module-04-remediated-pin.adoc[2.2].
...
. A human or bot runs xref:module-04-remediated-pin.adoc[2.2] steps: read `fixed`, pin, rebuild
...
Treat the poll as a *signal*, then reuse xref:module-04-remediated-pin.adoc[2.2]:
...
* Replacing xref:module-04-remediated-pin.adoc[2.2]’s scored OSV exercise for RHDP learners
