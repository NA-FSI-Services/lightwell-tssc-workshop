...
Core learners complete xref:module-04-osv.adoc[Module 4] with the seeded sample OSV and do *not* need this appendix. Use it to teach the *field PoV automation implication*: new remediated OSV files can trigger tickets or rebuild pipelines without binding the workshop to any customer ITSM product.
====
...
When the manifest checksum changes, something new (or updated) landed in the OSV path—often a new `fixed` event naming a `.rhlw-*` pin you can feed into Module 4’s evaluation loop.
...
. A human or bot runs Module 4 steps: read `fixed`, source-diff, pin, rebuild
...
Treat the poll as a *signal*, then reuse Module 4:
...
. Let Module 6 pipeline gates enforce the remediated pin before GitOps promotion

== Explicit non-goals

* Binding this workshop to a named ITSM, chat, or CMDB product
* Replacing Module 4’s seeded OSV exercise for RHDP learners
* Polling fictional channel names — only Validated / Remediated / Java OSV remotes apply

== Related

* Toolkit README: `tools/osv-eval/README.md`
* xref:module-04-osv.adoc[Module 4 — OSV triage and exact-version remediation]
