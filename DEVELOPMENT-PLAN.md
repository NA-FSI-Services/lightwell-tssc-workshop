...
3. **Student Git (Gitea)** — learners clone/push only in-cluster Gitea; never GitHub for app labs. When the app source lives under a monorepo subdirectory, seed/prepare Jobs isolate that path into a student repo at Gitea root ([`charts/components/gitea`](./charts/components/gitea/), [AGENTS.md](./AGENTS.md) Learner Git)
